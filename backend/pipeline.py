...
retrieval, and explanation logic lives in the individual modules. LLM
explanations are only requested for incidents scoring HIGH or CRITICAL
(total_score >= 60); everything below that band gets a deterministic
rule-based explanation instead.
...
def score_to_label(total_score: int) -> str:
    """Map a 0-100 composite score to a severity label band.

    Structural invariant shared with the frontend: >=80 critical,
    >=60 high, >=40 medium, else low.
    """
    if total_score >= 80:
        return "critical"
    if total_score >= 60:
...
def generate_rule_based_explanation(incident: Dict[str, Any]) -> ExplanationResult:
    ...
    return ExplanationResult(
        ...
        confidence_reason=(
            "Generated by deterministic rule-based analysis without LLM "
...
class ThreatPipeline:
    ...
    def process(self, raw_events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        working = [self._normalize_and_detect(raw) for raw in raw_events]
        ...
        for incident in incidents:
            try:
                summaries.append(self._process_incident(incident))
    ...
    def _normalize_and_detect(self, raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _process_incident(self, incident: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Step 6: LLM explanation for HIGH/CRITICAL only; deterministic
        # rule-based explanation otherwise (and whenever no LLM result).
        explanation: Optional[ExplanationResult] = None
        if (
            incident["total_score"] >= self.LLM_SCORE_THRESHOLD
